...
function jcreases(s)=let(mode=smode(s),d=fdata(legacy_tilespec(s[8],s[9],s[10])),h=180/s[8])(mode==8||mode==9||!rear_style)?[]:mode==5?[[1,0,d[1]]]:mode==6?[[0,1,0],[cos(h),sin(h),d[1]],[cos(h),-sin(h),d[1]]]:mode==7?[[cos(jalpha(s)),-sin(jalpha(s)),d[1]],[cos(jbeta(s)),-sin(jbeta(s)),fdata(legacy_tilespec(s[8],s[9],s[10]+1))[1]]]:[for(i=[0:s[8]/2-1])let(a=(i+.5)*360/s[8])[-sin(a),cos(a),0]];
