// PETAL parametric geometry kernel, version 1.2
// Units: millimetres. Front surface z = r*r / (4*f).
// Custom center interface: 120 mm OD, 30 mm center bore, four M4
// clearance windows on a 40 mm bolt circle. This is not a commercial standard.
...
function tilespec(n,k,j)=let(w=(diameter/2-45)/k,b0=45+j*w,b1=45+(j+1)*w,rm=b0+w*.65,h=180/n)
...
function envelope(s,t,a)=let(vs=[for(r=seq(s[0],s[1],16),ang=seq(s[2],s[3],24),l=[0,1])yawpoint(orient([r*cos(ang),r*sin(ang),(l?(rear_style?fz(r*cos(ang),fdata(s)):zz(r)-thickness):zz(r))],t),a)])
...
function pickprint(s,i=0)=let(angles=print_angle==-1?((supports||rear_style)?[60,55,50,45,65]:[0]):[print_angle])i>=len(angles)?[]:let(c=anglechoice(s,angles[i]))len(c)>0?c:pickprint(s,i+1);
function valid(n,k)=(diameter/2-45)/k>=40&&min([for(j=[0:k-1])len(pickprint(tilespec(n,k,j)))>0?1:0])==1;
...
function modelmesh(s)=let(M=splitall([verts(s),faces(s)],creases(s)),tops=[for(v=M[0])roof(s,v[0],v[1])],floor=min(tops)-s[6],mode=smode(s),d=mode==1?fdata(s):[]) [[for(i=[0:len(M[0])-1])let(v=M[0][i],bottom=mode==1?fz(v[0],d):mode>=2?floor:tops[i]-s[6])[v[0],v[1],tops[i]*(1-v[2])+bottom*v[2]]],M[1]];
...
module patch(s,t=0,printing=false,is_panel=false,bed_yaw=0){M=modelmesh(s);V=M[0];F=M[1];Vt=[for(v=V)orient(v,t)];lo=[for(k=[0:2])min([for(v=Vt)v[k]])];hi=[for(k=[0:2])max([for(v=Vt)v[k]])];
 ...
 if(printing&&is_panel&&supports){tip=s[0]*sin(s[3]);foot=min(8,2*tip/(rib_count-1)*.75);for(y=seq(-tip,tip,rib_count-1))rotate([0,0,rot])color("Orange")rib(s,t,(lo[0]+hi[0])/2,lo[2],y,foot);}
}
function side(j)=let(r=45+j*W+W*.65)[r-6,r+6,-14/r*180/PI,14/r*180/PI,[hh(r,-8/r*180/PI),hh(r,8/r*180/PI)],-thickness-.2,3.2,rear_style?2:0,N,K,j];
function radial(j)=let(r=45+(j+1)*W)[r-14,r+14,-6/r*180/PI,6/r*180/PI,[hh(r-8,0),hh(r+8,0)],-thickness-.2,3.2,rear_style?3:0,N,K,j];
function root_holes()=[for(i=[0:N-1])hh(52.5,i*360/N)];
function rear()=[15,60,-180/N,360-180/N,concat(root_holes(),[for(i=[0:3])hh(20,45+i*90)]),-thickness-.2,6,rear_style?4:0,N,K,0];
function clamp()=[43,60,-180/N,360-180/N,root_holes(),3.4,3.2];
module single(type,j=0,printing=true){s=type=="panel"?tilespec(N,K,j):type=="side-bridge"?side(j):type=="ring-bridge"?radial(j):type=="hub-rear"?rear():clamp();cp=type=="panel"&&printing?pickprint(s):[];t=type=="panel"&&printing?cp[3]:(type=="side-bridge"||type=="ring-bridge")&&!rear_style?atan((s[0]+s[1])/(4*diameter*fd)):0;if(type=="panel"&&printing)echo(print_angle=cp[1],bed_rotation=cp[2],support_ribs=supports?rib_count:0);patch(s,t,printing,type=="panel",type=="panel"&&printing?cp[2]:0);}
if(part=="assembly"){
