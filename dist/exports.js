...
${p.jointStyle&&p.adaptiveJoints?`Adaptive connector placement is ON, with a ${p.connectorSpacing} mm target spacing. Longer side seams and ring overlaps receive extra four-bolt saddles. The planner reserves space at ends and rejects crowded sockets. Spacing is a geometric construction rule; it is not derived from loads, material properties or wind.`:'Adaptive connector placement is OFF. Keyed joints use one saddle per side seam and one per neighboring-ring overlap.'}
