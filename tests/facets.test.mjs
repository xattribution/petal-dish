...
for(const cfg of [{},{diameter:800,facetAngle:15},{diameter:180,facetAngle:10}]){
 ...
 for(const p of m.parts.filter(p=>p.kind==='panel')){
  ...
  for(const [x,y,z]of p.mesh.v){const front=zAt(Math.hypot(x,y),m.p),back=facetZ(x,d);assert(front-back>=m.p.thickness-1e-7);assert(z<=front+1e-7);if(!p.spec.rootLip||Math.hypot(x,y)>66)assert(z>=back-1e-7);else assert(z>=p.spec.backFn(x,y)-2-1e-7,'Root thickening stays in its capture profile');}
