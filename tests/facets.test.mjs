...
for(const cfg of [{},{diameter:800,facetAngle:15},{diameter:180,facetAngle:10}]){
 ...
 for(const p of m.parts.filter(p=>p.kind==='panel')){
  ...
  for(const [x,y,z]of p.mesh.v){const front=zAt(Math.hypot(x,y),m.p),back=facetZ(x,d);assert(front-back>=m.p.thickness-1e-7);assert(z>=back-(m.p.jointStyle?3:0)-1e-7&&z<=front+1e-7);}
