...
for(const cfg of [{},{rearStyle:1},{diameter:800,rearStyle:1},{diameter:180,thickness:1.6,jointClearance:.1},{diameter:600,fd:.25,jointClearance:.4}]){
 ...
 for(const part of m.parts.filter(p=>p.kind==='bridge'||p.id==='hub-rear')){const lookup=intervals(part.mesh),angle=part.id.startsWith('side-')?Math.PI/n:0,c=Math.cos(angle),s=Math.sin(angle);const points=part.mesh.f.map(f=>[0,1].map(i=>f.reduce((sum,v)=>sum+part.mesh.v[v][i],0)/3));
 for(const[x,y]of points){const a=lookup(x,y);if(a.length<2)continue;const X=c*x-s*y,Y=s*x+c*y,theta=Math.atan2(Y,X),i=Math.round(theta/(2*Math.PI/n)),rot=i*2*Math.PI/n,u=Math.cos(rot)*X+Math.sin(rot)*Y,v=-Math.sin(rot)*X+Math.cos(rot)*Y;
 for(const ray of rays){const b=ray(u,v);if(b.length<2)continue;const gap=b[0]-a.at(-1);assert(gap>=-.002,`${part.id} collision ${gap} at ${x},${y}`);minGap=Math.min(minGap,gap);samples++;}}
 }
 assert(samples>100);assert.equal(m.bolts,n+4*n*k+4*n*(k-1));assert(jointsFit(m.p,n,k));
